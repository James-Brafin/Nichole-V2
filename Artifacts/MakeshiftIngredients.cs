 ...
 */
public class MakeshiftIngredients : Artifact, IRegisterable
{
    public static void Register(IPluginPackage<IModManifest> package, IModHelper helper)
    {
        helper.Content.Artifacts.RegisterArtifact(new ArtifactConfiguration
        {
            ...
            Name = ModEntry.Instance.AnyLoc.Bind(["artifact", "MakeshiftIngredients", "name"]).Localize,
            Description = ModEntry.Instance.AnyLoc.Bind(["artifact", "MakeshiftIngredients", "desc"]).Localize,
            /*
             * For Artifacts with just one sprite, registering them at the place of usage helps simplify things.
             */
            Sprite = helper.Content.Sprites.RegisterSprite(package.PackageRoot.GetRelativeFile("assets/artifacts/MakeshiftIngredients.png")).Sprite
        });
    }
    ...
    public override void OnReceiveArtifact(State state)
    {
        state.GetCurrentQueue().QueueImmediate(
            ...
            }.ApplyModData(CardBrowseFilterManager.FilterReagentKey, false)
        );
    }
}